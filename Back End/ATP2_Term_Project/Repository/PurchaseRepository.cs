...
namespace ATP2_Term_Project.Repository
{
    public class PurchaseRepository : Repository<Purchase>, IPurchaseRepository
    {
        ...
        public Purchase GetPurchaseByProduct(int id)
        {
            ...
            if (purchase != null)
            {
                purchase.HyperLinks.Add(new HyperLink() { HRef = "http://localhost:6100/api/infos/" + purchase.InformationId, HttpMethod = "GET", Relation = "Self" });
                purchase.HyperLinks.Add(new HyperLink() { HRef = "http://localhost:6100/api/infos", HttpMethod = "POST", Relation = "Create a new Purchase resource" });
                purchase.HyperLinks.Add(new HyperLink() { HRef = "http://localhost:6100/api/infos/" + purchase.InformationId, HttpMethod = "PUT", Relation = "Edit a existing Purchase resource" });
                purchase.HyperLinks.Add(new HyperLink() { HRef = "http://localhost:6100/api/infos/" + purchase.InformationId, HttpMethod = "DELETE", Relation = "Delete a existing Purchase resource" });
