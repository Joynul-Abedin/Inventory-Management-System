...
namespace ATP2_Term_Project.Repository
{
    public class SaleseRepository : Repository<Sale>,ISalesRepository{
        ...
        public Sale GetSalesByProduct(int id)
        {
            ...
            if (sale != null)
            {
                sale.HyperLinks.Add(new HyperLink() { HRef = "http://localhost:6100/api/infos/" + sale.InformationId, HttpMethod = "GET", Relation = "Self" });
                sale.HyperLinks.Add(new HyperLink() { HRef = "http://localhost:6100/api/infos", HttpMethod = "POST", Relation = "Create a new Sale resource" });
                sale.HyperLinks.Add(new HyperLink() { HRef = "http://localhost:6100/api/infos/" + sale.InformationId, HttpMethod = "PUT", Relation = "Edit a existing Sale resource" });
                sale.HyperLinks.Add(new HyperLink() { HRef = "http://localhost:6100/api/infos/" + sale.InformationId, HttpMethod = "DELETE", Relation = "Delete a existing Sale resource" });
